...
async function run() {
  const speakerText = await parsePdf('/Users/kuzeykalyoncu/Desktop/3. BAŞKENT OPEN Speaker Tab.docx.pdf');
  const teamText = await parsePdf('/Users/kuzeykalyoncu/Desktop/3. BAŞKENT OPEN Team Tab.docx.pdf');
  
  console.log("--- Speaker Tab ---");
  console.log(speakerText.split('\n').filter(l => l.includes("İbrahim") || l.includes("Kutay")).join('\n'));
  
  console.log("--- Team Tab ---");
  console.log(teamText.split('\n').filter(l => l.includes("ODTÜ") || l.includes("Mutemet")).join('\n'));
}
